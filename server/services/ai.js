...
const TEXT_PARSE_PROMPT = (referenceTime) => `当前时间是 ${referenceTime}。
...
注意事项：
1. 如果用户说"明天下午三点开会"，start_time 应该是明天下午三点。
2. 如果用户说"大概1小时"，end_time 应该是 start_time + 1小时。
3. 如果用户没提提醒时间，remind_at 设为 start_time 前15分钟。
4. 只返回 JSON，不要有任何其他文字或 markdown 代码块。`;

const IMAGE_PARSE_PROMPT = (referenceTime) => `当前时间是 ${referenceTime}。
...
注意事项：
1. 仔细识别图片中的日期和时间，转换为 ISO 8601 格式。
2. 如果图片只有日期没有具体时间，start_time 设为该日期的上午9点。
3. 如果是航班，start_time 为起飞时间，end_time 为降落时间（若有）。
4. 如果是演唱会/活动，start_time 为开场时间。
5. 如果图片中没有明确年份，假设为今年；如果日期已过，假设为明年。
6. remind_at 没提则默认为 start_time 前2小时。
